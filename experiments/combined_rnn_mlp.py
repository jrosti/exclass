...
def train(epochs=300, batch_size=50, learning_rate=.001, layers=[100, 80, 50, 30], mlp_act=tf.nn.relu,
          hidden_state_size=100, dropout_keep_prob=0.9):
    ...
    mlp_logits = layer(hidden_layers[-1], dataset.num_labels, act=tf.identity, name='mlp_logits')

    rnn_input, _, state = build_rnn(dataset.data.max_time, dataset.data.word_dim, hidden_state_size=100)
    ...
    for step in range(1000000):
        ...
        if step % 100 == 0:
            ...
            valy_, lossv = sess.run([outp, loss], {rnn_input: sentences_valid, inp_labels: labels_valid,
                                                   mlp_inp: mlp_valid, keep_prob: 1.0})
            print("s={} e={} t_err={:.3f} v_err={:.3f} losst={:.3f} lossv={:.3f}".format(step, dataset.epoch,
